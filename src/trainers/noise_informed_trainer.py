...
class NoiseInformedTrainer(BaseTrainer, TensorboardLoggerMixin):
    def __init__(self, **kwargs):
        ...
        log_name = kwargs.get('save_dir', "logs/noise")
        self.checkpoint_prefix = "noise"
    ...
    def cons_vol_loss(self, pred_x):
        ...
        in_domain = torch.all(in_domain, dim=-1).float().requires_grad_()
    ...
    def predict(self, inputs):
        ...
        xt = odeint(self.model, s_x0, t, method='fehlberg2', atol=1e-4, rtol=1e-4)
        xt_c = odeint(self.model.cons_vector_field, s_x0, t, method='fehlberg2', atol=1e-4, rtol=1e-4)
        return xt, xt_c
    ...
    def _log_and_checkpoint(self,
        ...
        val_loss, val_KL_w, val_nll = self.validate(**validate_kwargs)
    ...
    def train(self, **kwargs):
        for epoch in range(1, self.epochs + 1):
            ...
            for batch_idx, batch in enumerate(self.data_loader):
                ...
                loss, neg_loglike, KL_w = self.compute_loss(outputs, targets)
                loss.backward()
                self.optimizer.step()
            ...
            if self.rank == 0:
                ...
                if epoch % self.log_interval == 0:
                    ...
                    self._log_and_checkpoint(epoch, loss, KL_w=KL_w, neg_loglike=neg_loglike)
    ...
    def validate(self):
        ...
        val_losses, klws, nlls = [], [], []
        for val_batch in self.val_loader:
            (batch_x0, batch_t), batch_ys = self.unpack_batch(val_batch)
            with torch.no_grad():
                pred_val, pred_val_c = self.predict((batch_x0, batch_t))
                ...
                val_loss = val_nll + val_KL_w # + val_KL_x0
        ...
        return val_loss, val_KL_w, val_nll
